---
title: "Modelo VAR"
author: "R Ballini"
output:
  word_document: default
  html_document: default
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```


# Carregar os Pacotes
```{r}
library(MASS)
library(lmtest)
library(vars)
library(urca)
library(readr)
library(graphics)
```

# Leitura dos Dados

```{r}
#Dados=read_csv('DadosBueno.csv',col_names=T)
#View(Dados)

Dados=ts(DadosBueno[,2:17],start=c(1994,9),freq=12)
View(Dados)

layout(matrix(1:4,nrow=2,ncol=2))
plot(Dados[,1],main='Taxa de Juros',ylab='',xlab='')
plot(Dados[,2],main='Câmbio',ylab='',xlab='')
plot(Dados[,3],main='ADM',ylab='',xlab='')
plot(Dados[,4],main='Livres',ylab='',xlab='')

# Necessário testes de RU
```

# Modelo VAR
```{r}
ordem=VARselect(Dados[,1:4],lag.max=8,type='none',exogen=Dados[,5:16])
ordem
```

# Estimação Modelo
```{r}
modelo1=VAR(Dados[,1:4],p=1,type='const',exogen=Dados[,5:16])
summary(modelo1)

summary(modelo1,equation='Livres')

```

# Diagnóstico do Modelo
```{r}
# Raízes do Sistema
roots(modelo1,modulus=T)

roots(modelo1,modulus=F)

plot(stability(modelo1,type='OLS-CUSUM'))

```


```{r}
layout(matrix(1:4,nrow=2,ncol=2))
acf(residuals(modelo1)[,1],main='Livres')
acf(residuals(modelo1)[,2],main='ADM')
acf(residuals(modelo1)[,3],main='Câmbio')
acf(residuals(modelo1)[,4],main='Juros')

```

```{r}
#Correlação Cruzada
layout(matrix(1:4,nrow=2,ncol=2))
ccf(residuals(modelo1)[,1],residuals(modelo1)[,2],lag.max=16,main='LivresXADM')
ccf(residuals(modelo1)[,1],residuals(modelo1)[,3],lag.max=16,main='LivresXCâmbio')
ccf(residuals(modelo1)[,1],residuals(modelo1)[,4],lag.max=16,main='LivresXJuros')

```

```{r}
# Teste de Autocorrelação

# Teste de Portmanteau - clássico
teste.pt.asy=serial.test(modelo1,lags.pt=12,type='PT.asymptotic')
teste.pt.asy
plot(teste.pt.asy)

# Teste de Portmanteau - ajustado
teste.pt.adj=serial.test(modelo1,lags.pt=12,type='PT.adjusted')
teste.pt.adj
plot(teste.pt.adj)

```

```{r}
# Teste LM 
teste.lm=serial.test(modelo1,lags.bg=12,type='BG')
teste.lm
plot(teste.lm)

```

```{r}
# teste ARCH-LM
teste.arch=arch.test(modelo1,multivariate.only = F)
teste.arch

```

```{r}
#Teste de Normalidade
teste.norm=normality.test(modelo1,multivariate.only = F)
teste.norm
```
